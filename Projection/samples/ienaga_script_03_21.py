...
def image_align(src_file, lm, enable_padding=False):
    ...
    # Transform.
    img = img.transform(img.size, Image.QUAD, (quad + 0.5).flatten(), Image.BILINEAR)
...
if __name__ == '__main__':
    ...
    for img_path in img_paths:
        # img_path = '../custom_ds/raw_images/naoto7.png'
        st = time.time()
        ...
        img.save(f'{output_dir}aligned_{os.path.basename(img_path)}')
        # continue
        ...
        np.save(f'{output_dir}{os.path.basename(img_path).split(".")[0]}.npy', latents)
        # break

    total_time = time.time() - all_st
    print(f'{total_time:.2f}, {total_time/len(img_paths):.2f}')
